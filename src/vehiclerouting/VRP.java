...
//        Random ran = new Random(151190);
        ...
        //Problem Parameters
        int NoOfCustomers = 500;
        int NoOfVehicles = 150;
        int VehicleCap = 4;
        ...
        for (int i = 1; i <= NoOfCustomers; i++) {
            Nodes[i] = new Node(i, //Id ) is reserved for depot
                    ...
                    ran.nextInt(100),
                    1,//4 + ran.nextInt(4)  //Random Demand
                    false // new is driver
            );
        }
        ...
        Solution s = new Solution(NoOfCustomers, NoOfVehicles, VehicleCap, Nodes);
...
class Solution
{
    ...
    Solution(int CustNum, int VechNum , int VechCap, Node[] Nodes)//pass in distance mx
    {
        ...
        {
        	int j = i + 1;
            Vehicles[i] = new Vehicle(j,VechCap, j);
            VehiclesForBestSolution[i] = new Vehicle(j,VechCap, j);
            Nodes[j].isDirver = true;
            Nodes[j].IsRouted = true;
//            Nodes[j].NodeId = 0;
        }
    ...
    public boolean UnassignedCustomerExists(Node[] Nodes)
    {
        ...
        {
            if (!Nodes[i].isDirver && !Nodes[i].IsRouted)
                return true;
        }
        return false;
    }
    ...
    public void GreedySolution(Node[] Nodes , double[][] CostMatrix) {
        ...
        while (UnassignedCustomerExists(Nodes)) {
            ...
            {
                Vehicles[VehIndex].AddNode(Nodes[VehIndex + 1]);
            }

            for (int i = 1; i <= NoOfCustomers; i++) {
                if (!Nodes[i].isDirver && Nodes[i].IsRouted == false) {
...
//                            System.out.println(RouteTolength + " " + j);
//                            System.out.println(RouteFrom.get(j));
                            RouteFrom.get(i);
//                            if (RouteFrom.get(i).isDirver || RouteFrom.get(j).isDirver) continue;
...
//            System.out.println("Route from size: " + RouteFrom.size());
//            System.out.println("Swapidx A + 1: " + SwapIndexA);
        ...
        try{
//            String dirPath = "C:\\Users\\RAZER\\eclipse-workspace\\VehicleRouting\\pic";
//            File f = new File(dirPath + File.separator + fileName);
//            PrintWriter writer = new PrintWriter("PastSolutionsTabu.txt", "UTF-8");
//        	String fileName = "PastSolutionsTabu.txt";
            PrintWriter writer = new PrintWriter("C:\\Users\\RAZER\\eclipse-workspace\\VehicleRouting\\pic\\PastSolutionsTabu.txt", "UTF-8");
    ...
    public void InterRouteLocalSearch(Node[] Nodes,  double[][] CostMatrix) {
        ...
        try{
            PrintWriter writer = new PrintWriter("C:\\Users\\RAZER\\eclipse-workspace\\VehicleRouting\\pic\\PastSolutionsInter.txt", "UTF-8");
            for  (int i = 0; i< PastSolutions.size(); i++){
                writer.println(PastSolutions.get(i)+"\t");
            }
            writer.close();
    ...
    public void IntraRouteLocalSearch(Node[] Nodes,  double[][] CostMatrix) {
        ...
        try{
            PrintWriter writer = new PrintWriter("C:\\Users\\RAZER\\eclipse-workspace\\VehicleRouting\\pic\\PastSolutionsIntra.txt", "UTF-8");
            for  (int i = 0; i< PastSolutions.size(); i++){
                writer.println(PastSolutions.get(i)+"\t");
            }
            writer.close();
...
class Node
{
    ...
    public boolean isDirver;//new check if its driver
    ...
    public Node(int depot_x,int depot_y) //Cunstructor for depot
    {
        ...
        this.Node_Y = depot_y;
        this.IsDepot = true;
        this.isDirver = true; // NEW
        this.demand = 50;
    }

    public Node(int id ,int x, int y, int demand, boolean isDriver) //Cunstructor for Customers
    {
        this.NodeId = id;
        this.Node_X = x;
        this.Node_Y = y;
        ...
        this.IsRouted = false;
        this.IsDepot = false;
        this.isDirver = isDriver;//new is driver
    }
}
...
class Vehicle
{
    ...
    public Vehicle(int id, int cap, int curLoc)
    {
        this.VehId = id;
        this.capacity = cap;
        this.load = 0;
        this.CurLoc = curLoc; //In depot Initially new pass in curloc
        this.Closed = false;
        this.Route.clear();
    }
